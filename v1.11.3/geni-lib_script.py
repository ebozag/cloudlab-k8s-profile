...
###Known issues
1. This profile uses Ubuntu 16.04 instead of 18.04 because kubernetes does not have a bionic source yet, this will be upgraded when Kubernetes bionic is ready.
...
kube_m.addService(pg.Execute(shell="bash", command="/mnt/extra/master.sh v1.11.3"))
slave_ifaces = []
for i in range(1,params.computeNodeCount+1):
    ...
    kube_s.addService(pg.Execute(shell="bash", command="/mnt/extra/slave.sh v1.11.3"))
